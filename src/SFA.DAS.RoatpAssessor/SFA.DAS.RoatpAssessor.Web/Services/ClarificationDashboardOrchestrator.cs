...
namespace SFA.DAS.RoatpAssessor.Web.Services
{
    public class ClarificationDashboardOrchestrator : IClarificationDashboardOrchestrator
    {
        ...
        public async Task<InClarificationApplicationsViewModel> GetInClarificationApplicationsViewModel(string userId, string searchTerm, string sortColumn, string sortOrder)
        {
            var applicationSummary = await _applicationApiClient.GetApplicationCounts(userId, searchTerm);
            var applications = await _applicationApiClient.GetInClarificationApplications(userId, searchTerm, sortColumn, sortOrder);
